...
public class RxExample {
	...
	public static void main(String[] args) {
		...
		search.subscribe(
				order -> System.out.println("Order type " + order.getOrderType() + " value " + order.getAmount()));
	...
	public Observable<Order> search(OrderType typeTosearch, int maxResults) {
		return getOrderType().filter(type -> type == typeTosearch).flatMap(this::lookupArtist).take(maxResults);
	}
