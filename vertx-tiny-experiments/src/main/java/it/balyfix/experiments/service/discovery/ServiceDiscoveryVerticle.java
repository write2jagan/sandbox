package it.balyfix.experiments.service.discovery;
...
public class ServiceDiscoveryVerticle extends AbstractVerticle {
  ...
  public void start() {
   ...
   ServiceDiscovery discovery = ServiceDiscovery.create(vertx,
    ...
    String usageAddress = discovery.options().getUsageAddress();
    
    
    System.out.println(usageAddress);
    String announceAddress = discovery.options().getAnnounceAddress();
    System.out.println(announceAddress);
    ...
    discovery.publish(record2, ar -> {
      if (ar.succeeded()) {
        ...
        System.out.println("record 2 not Published");
      }
    });
    ...
    discovery.getRecord(r -> r.getName().equals(record2.getName()), ar -> {
      if (ar.succeeded()) {
        if (ar.result() != null) {
          ...
          client.getNow("localhost","/api", response -> {
            //release the service
        	response.bodyHandler(bh -> 
        	{
        		System.out.println("il corpo del messaggio " + bh.toString());
